...
const digitalImages = [
  '/digital/DSCF1902.JPG',
  '/digital/DSCF1861.JPG',
]
...
function Home({ current, nextImage, prevImage }: { current: number, nextImage: () => void, prevImage: () => void }) {
  const [digitalCurrent, setDigitalCurrent] = useState(0)
  const nextDigitalImage = () => setDigitalCurrent((digitalCurrent + 1) % digitalImages.length)
  const prevDigitalImage = () => setDigitalCurrent((digitalCurrent - 1 + digitalImages.length) % digitalImages.length)
  
  const [filmCurrent, setFilmCurrent] = useState(0)
  const nextFilmImage = () => setFilmCurrent((filmCurrent + 1) % filmImages.length)
  const prevFilmImage = () => setFilmCurrent((filmCurrent - 1 + filmImages.length) % filmImages.length)
  return (
    <div>
      <Navbar />
      {/* 图片轮播大框 */}
      <div className="w-screen h-[40rem] relative">
        <div className="relative w-screen h-[40rem] overflow-hidden flex items-center">
          <button onClick={prevImage} className="absolute left-2 top-1/2 -translate-y-1/2 bg-white bg-opacity-70 hover:bg-opacity-100 rounded-full p-2 shadow transition z-10">
          ...
          <img src={images[current]} alt="preview" className="w-screen h-[40rem] object-cover object-center transition-all duration-500" style={{zIndex: 0}} />
          <button onClick={nextImage} className="absolute right-2 top-1/2 -translate-y-1/2 bg-white bg-opacity-70 hover:bg-opacity-100 rounded-full p-2 shadow transition z-10">
      ...
      <div className="w-screen h-[40rem] relative overflow-hidden">
        ...
        <div className="absolute left-0 top-0 w-1/2 h-full flex items-center justify-center">
          <img 
            ...
            style={{zIndex: 0}}
          />
          <div className="relative z-10 flex items-center justify-center" style={{width: '28rem', height: '18rem', transform: 'translateY(35%) translateX(-22%)'}}>
            <div className="bg-white border-0 rounded-xl shadow-xl overflow-hidden w-full h-full flex items-center justify-center">
              {/* 小型轮播 */}
        ...
        <div className="absolute right-0 top-0 w-1/2 h-full bg-white flex items-center justify-center">
          <div className="text-center space-y-6">
            <div>
              <h2 className="text-5xl font-bold text-gray-900 mb-2">Digital Photography</h2>
      ...
      {/* 第三个区域 - 胶片风格 */}
      <div className="w-screen h-[40rem] relative overflow-x-auto overflow-y-hidden" style={{
        background: 'linear-gradient(180deg, #a97c50 0%, #e2c9a0 50%, #a97c50 100%)',
        boxShadow: 'inset 0 8px 32px #6b3e1b, inset 0 -8px 32px #6b3e1b'
      }}>
        {/* 竖排标题 */}
        <div className="absolute left-0 top-1/2 -translate-y-1/2 z-30" style={{height: '80%', display: 'flex', alignItems: 'center'}}>
          <span style={{
        ...
        {/* 竖排右侧按钮 */}
        <div className="absolute right-0 top-1/2 -translate-y-1/2 z-30" style={{height: '80%', display: 'flex', alignItems: 'center'}}>
          <button
            style={{
              writingMode: 'vertical-rl',
              textOrientation: 'mixed',
            ...
            onMouseOver={e => { e.currentTarget.style.background = '#a97c50'; e.currentTarget.style.color = '#6b3e1b'; }}
            onMouseOut={e => { e.currentTarget.style.background = '#6b3e1b'; e.currentTarget.style.color = '#fff'; }}
          >
            Browse All
          </button>
        </div>
        ...
        <div className="relative z-20 flex items-center justify-center h-full">
          <div className="flex flex-row items-center justify-center w-[160vw] h-[28rem] gap-4">
            {filmImages.slice(0, 3).map((img, idx) => (
              <div key={idx} className="bg-[#e2c9a0] border-4 border-[#a97c50] rounded-lg shadow-lg h-full flex items-center justify-center overflow-hidden">
                <img src={img} alt={`film-${idx}`} className="h-full w-auto max-w-none" />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="w-screen h-[40rem] flex flex-row">
        {[
          {img: '/seasons/Spring.jpg', label: 'Spring'},
          {img: '/seasons/Summer.jpg', label: 'Summer'},
          {img: '/seasons/Autumn.jpg', label: 'Autumn'},
          {img: '/seasons/Winter.JPG', label: 'Winter'},
        ].map((item, idx) => (
          <div key={idx} className="w-1/4 h-full relative overflow-hidden">
            <img src={item.img} alt={`season-${idx}`} className="w-full h-full object-cover" />
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <span className="text-white text-5xl font-extrabold drop-shadow-lg tracking-widest" style={{textShadow: '0 4px 24px #000, 0 1px 2px #000'}}> 
                {item.label}
              </span>
            </div>
          </div>
      ...
      <div className="w-screen h-[40rem] bg-white flex items-center justify-between px-24">
        ...
        {/* 右侧图片及小轮播 */}
        <div className="relative h-full flex items-center justify-center" style={{maxWidth: '48rem', width: '100%'}}>
          {/* 小型轮播叠加在图片上方 */}
          <div className="absolute left-1/2 top-1/2 z-20" style={{width: '15rem', height: '15rem', transform: 'translate(-40%, -60%)'}}>
            {/* 半透明渐变边缘 */}
          ...
          {/* 右侧大图片 */}
          <img src="/images/hasselblad.jpg" alt="hasselblad" className="h-full rounded-2xl object-cover" style={{maxWidth: '48rem'}} />
        </div>
      </div>
      {/* 新区域：与春夏秋冬区域尺寸一致 */}
      <div className="w-screen h-[40rem] flex items-center justify-center bg-white px-8 gap-12">
        ...
        <div className="flex flex-col items-center justify-center h-[25.6rem] w-[18rem]">
          <div className="text-center space-y-6">
            <div>
              <h2 className="text-5xl font-bold text-gray-900 mb-2">Portrait</h2>
            </div>
            <button className="px-8 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors font-medium text-lg">
              Browse All
            </button>
        ...
        <div className="flex items-center justify-center h-[25.6rem] w-1/3">
          <div className="relative h-full w-full flex items-center justify-center"
            style={{
              borderRadius: '1.5rem',
              boxShadow: '0 12px 48px #b88654, 0 4px 16px #7c5a36, 0 0 0 16px #6b3e1b',
            ...
            <div style={{
              ...
              <div style={{
                ...
                <img src="/portrait/000414710005.jpg" alt="portrait" style={{width: '100%', height: '100%', objectFit: 'cover'}} />
              </div>
            </div>
          </div>
        </div>
    ...
    <>
      <Router>
        <Routes>
          ...
          <Route path="*" element={<Home current={current} nextImage={nextImage} prevImage={prevImage} />} />
        </Routes>
      </Router>
      <Footer />
    </>
